...
# Loading data
data <- read.table("GSE63734_SCZ_Neurons.ucsc.Clean.txt", header=T)
...
for (i in 1:nrow(data)){
    if (prod(data[i,2:12]) != 0){
        data.selected <- rbind(data.selected, data[i,])
    }
}

#Separate data for case and control
case <- data.selected[,c(2:6)]
control <- data.selected[,c(7:12)]

#Calculate FoldChange
foldchange <- apply(case, 1, mean)/apply(control, 1, mean)

#Calculate P-value
p.values <- apply(cbind(case,control),1,function(x) {t.test(x[1:5],x[6:11])$p.value})
